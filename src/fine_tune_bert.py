...
maxlen = 25
config_path = '../data/baidu_ernie/config.json'
checkpoint_path = '../data/baidu_ernie/bert_model.ckpt'
dict_path = '../data/baidu_ernie/vocab.txt'
...
def train(tokenizer):
    ...
    train_data = [data[j] for i, j in enumerate(random_order) if i <= int(len(data) * 0.8)]
    valid_data = [data[j] for i, j in enumerate(random_order) if i > int(len(data) * 0.8)]

    train_D = data_generator(train_data, tokenizer)
    valid_D = data_generator(valid_data, tokenizer)

    epochs = 10
    # models.load('./ernie_{}.h5'.format(i), custom_objects=get_custom_objects())
...
save_path = saver.save(sess, "./models.ckpt")